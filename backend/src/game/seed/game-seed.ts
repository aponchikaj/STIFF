...
export async function seedGame(manager: EntityManager): Promise<SeedReport> {
  /**
   * Approval normally means a human looked at a chart, and the schema enforces
   * that by refusing an approved row without an approver and a timestamp.
   *
   * For these eight it is not a rubber stamp. They are generated by
   * `authorChart`, whose own test suite asserts every invariant approval is
   * meant to check — NPS caps at five tempos, gap floors, no jacks on easy,
   * integer note times — and running this seed with `SEED_GAME=yes` is the
   * deliberate act. Uploaded and AI-generated charts still land as drafts and
   * still need a person.
   */
  ...
  const songs = await upsertAll(
    ...
    songSpecs.map((s) => ({
      ...
      bpmIsManual: true,
      // Null on purpose — no stems exist. The client synthesises these two
      // metronomes locally from the song's tempo, which is why `ready` is
      // honest here: there is nothing missing that play needs.
      audioInstKey: null,
      audioVoicesKey: null,
  ...
  for (const song of songs) {
    ...
    for (const difficulty of DIFFICULTIES) {
      ...
      const row = repo.create({
        ...
        // Approved only when there is a real admin to attribute it to; the
        // CHECK constraint refuses the row otherwise, and inventing an id to
        // satisfy it would be exactly the lie the constraint exists to stop.
        ...(approver
          ? {
