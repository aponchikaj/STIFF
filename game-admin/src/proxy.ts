...
/**
 * Where the game keeps hand-ins.
 *
 * A player's photo or clip lives in the R2 bucket behind `media.stiff.ge`.
 * The review screen cannot show a single one of them unless that origin is
 * named here, and a wrong value fails silently — the image simply never
 * paints. An env var rather than a constant, because staging points at a
 * different bucket.
 *
 * Cloudinary is deliberately absent: it holds the shop's product photography,
 * and nothing on this panel has any business loading it.
 */
function gameMediaOrigin(): string | null {
  const raw = process.env.NEXT_PUBLIC_GAME_MEDIA_URL?.trim();
  if (!raw) return "https://media.stiff.ge";
  try {
    const url = new URL(raw);
...
export function proxy(request: NextRequest) {
  ...
  const media = gameMediaOrigin();
  const mediaSrc = media ? ` ${media}` : "";
  ...
  const csp = [
    ...
    `img-src 'self' blob: data:${mediaSrc}`,
    `media-src 'self' blob:${mediaSrc}`,
    "font-src 'self' data:",
    `connect-src 'self' ${backendOrigin()}${mediaSrc}`,
    "object-src 'none'",
    "base-uri 'none'",
    "form-action 'self'",
