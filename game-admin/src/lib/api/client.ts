 ...
 * Cookies only.
 *
 * The shop's own client keeps tokens in localStorage as a fallback for
 * browsers that block third-party cookies. This one deliberately does not:
 * /api is proxied first-party from admin.stiff.co, so the fallback would buy
 * nothing, and a token readable by script is a token an XSS can walk off
 * with. The session lives entirely in httpOnly cookies the page cannot see.
 */
