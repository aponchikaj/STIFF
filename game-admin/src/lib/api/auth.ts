...
/**
 * The admin session — the same one admin.stiff.ge uses.
 *
 * This panel is a separate origin and a separate deployment, but not a
 * separate identity: an admin is an ordinary shop account with `role=admin`,
 * and `/api/admin/auth/*` is the one place that mints an admin-audience
 * token. That token reaches `/api/game/admin/*` because those routes carry
 * `@Roles('admin')`.
 *
 * Separate endpoints from the shop's own `/auth/*` and a separate cookie
 * pair, so signing in here neither creates nor consumes a shopper's session.
 * There is no register, no password reset and no email verification: admins
 * are made by promoting an existing account, never by signing up here.
 */
